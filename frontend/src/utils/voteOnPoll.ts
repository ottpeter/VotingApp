...
export async function voteOnPoll(pollID: PollId, optionIndex: number) {
  ...
  contract.vote(pollID, optionIndex)
    ...
    .catch((err: Error) => console.error(`There was an error while trying to vote on proposal ${pollID}, optionIndex: ${optionIndex}`, err));
}